...
class FaceFilterApp:
    ...
    def draw_sunglasses(self, frame, landmarks):
        try:
            ...
            # Draw sunglasses with increased size
            cv2.ellipse(
                frame,
                (int((left_eye_center[0] + right_eye_center[0]) / 2),
                 int((left_eye_center[1] + right_eye_center[1]) / 2)),
                (int(abs(right_eye_center[0] - left_eye_center[0]) * 1.2),  # Increased width
                 int(abs(right_eye_center[1] - left_eye_center[1]) * 0.8)),  # Increased height
                0, 0, 360, (0, 0, 0), -1
            )
            print("Sunglasses drawn")
        except Exception as e:
            print(f"Error drawing sunglasses: {e}")
    ...
    def draw_hat(self, frame, landmarks):
        try:
            ...
            # Draw hat with increased size
            points = np.array([
                [int(left_eye[0] - 40), int(forehead[1] - 60)],  # Increased width and height
    ...
    def draw_mustache(self, frame, landmarks):
        try:
            ...
            # Draw mustache with increased size
            points = np.array([
                [int(mouth[0][0]), int(mouth[0][1])],
                [int((mouth[0][0] + mouth[6][0]) / 2), int(mouth[0][1] + 20)],  # Increased height
                [int(mouth[6][0]), int(mouth[6][1])]
            ], np.int32)
            cv2.polylines(frame, [points], False, (0, 0, 0), 5)  # Increased thickness
            print("Mustache drawn")
        except Exception as e:
            print(f"Error drawing mustache: {e}")
